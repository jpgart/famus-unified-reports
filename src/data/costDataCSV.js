...
// Calculate metrics from CSV data using consolidated initial stock
export const calculateMetricsFromCSV = async () => {
  if (cachedMetrics) return cachedMetrics;
  
  const chargeData = await loadCostDataFromCSV();
  const consolidatedStock = await getConsolidatedInitialStock();
  ...
  const excludedExporters = ['Videxport'];
  ...
  chargeData
    ...
    .forEach(row => {
      ...
      if (!lotidTotals[lotid]) {
        // Get consolidated initial stock for this Lotid
        const stockInfo = consolidatedStock[lotid];
        const totalBoxes = stockInfo ? stockInfo.totalStock : 0;
        
        lotidTotals[lotid] = {
          lotid,
          exporter: row['Exporter Clean'],
          totalChargeAmount: 0,
          totalBoxes, // Use consolidated stock from detailed CSV
          charges: []
        };
      }
  ...
  console.log(`✅ Calculated metrics for ${Object.keys(lotidTotals).length} lot IDs from CSV using consolidated initial stock`);
  return lotidTotals;
};
...
export const getInitialStockAnalysis = async () => {
  ...
  // Calculate unique varieties and exporters for the summary
  analysis.uniqueVarieties = Object.keys(analysis.byVariety).length;
  analysis.uniqueExporters = Object.keys(analysis.byExporter).length;
  
  return analysis;
};

// Function to get top varieties by stock volume
...
// Function to get stock distribution by month
...
export const analyzeSpecificChargeFromCSV = async (chargeType, displayName) => {
  ...
  // Calculate unique boxes per exporter using consolidated stock
  ...
  filteredData.forEach(row => {
    ...
    if (!processedLotids.has(lotid)) {
      const stockInfo = consolidatedStock[lotid];
      const stock = stockInfo ? stockInfo.totalStock : 0;
      
      if (!exporterUniqueBoxes[exporter]) {
        exporterUniqueBoxes[exporter] = 0;
      }
      exporterUniqueBoxes[exporter] += stock;
      totalBoxesUnique += stock;
  ...
  filteredData.forEach(row => {
    ...
    if (!exporterStats[exporter]) {
      exporterStats[exporter] = {
        exporter,
        totalAmount: 0,
        totalBoxes: exporterUniqueBoxes[exporter] || 0,
        recordCount: 0
      };
    }
